...
def retrieve_rag_context_icl(submission_segment: str ,exercise_id: int) ->str:
    ...
    metadata = query_embedding(query_submission,exercise_id)
    print(type(metadata))
    if metadata is not None:
        for data in metadata:
            rag_context.append({"submission_chunk": submission_segment, "feedback": data})
    
    else:
        return( "There are no submission at the moment")
        
    return format_rag_context(rag_context)

    # full_return = []
    # for data in metadata:
    #     if data["index_start"] is not None and data["index_end"] is not None:
    #         reference = 
    #     pass
    # return metadata
...
def format_rag_context(rag_context):
    ...
    for context_item in rag_context:
        ...
        if feedback is not None:
            ...
            # feedback["text_reference"] = feedback["text_reference"]#get_reference(feedback, submission_text)
            clean_feedback = {key: value for key, value in feedback.items() if key not in ["id","index_start","index_end","is_graded","meta","exercise_id", "submission_id"]} 
